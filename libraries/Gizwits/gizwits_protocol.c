...
static int8_t ICACHE_FLASH_ATTR gizDataPoint2Event(gizwitsIssued_t *issuedData, eventInfo_t *info, dataPoint_t *dataPoints)
{
    ...
    if(0x01 == issuedData->attrFlags.flagmessage_test)
    {
        info->event[info->num] = EVENT_message_test;
        info->num++;
        dataPoints->valuemessage_test = gizX2Y(message_test_RATIO,  message_test_ADDITION, issuedData->attrVals.valuemessage_test); 
    }
...
static int8_t ICACHE_FLASH_ATTR gizCheckReport(dataPoint_t *cur, dataPoint_t *last)
{
    ...
    if(last->valuemessage_test != cur->valuemessage_test)
    {
        GIZWITS_LOG("valuemessage_test Changed\n");
        ret = 1;
    }

    if(last->valueT_test != cur->valueT_test)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueT_test Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
    if(last->valueH_test != cur->valueH_test)
    {
        if(gizGetTimerCount()-lastReportTime >= REPORT_TIME_MAX)
        {
            GIZWITS_LOG("valueH_test Changed\n");
            lastReportTime = gizGetTimerCount();
            ret = 1;
        }
    }
...
static int8_t ICACHE_FLASH_ATTR gizDataPoints2ReportData(dataPoint_t *dataPoints , devStatus_t *devStatusPtr)
{
    ...
    devStatusPtr->valuemessage_test = gizY2X(message_test_RATIO,  message_test_ADDITION, dataPoints->valuemessage_test); 
    devStatusPtr->valueT_test = gizY2X(T_test_RATIO,  T_test_ADDITION, dataPoints->valueT_test); 
    devStatusPtr->valueH_test = gizY2X(H_test_RATIO,  H_test_ADDITION, dataPoints->valueH_test); 
